...
% muscle parameters for the equivalent muscle in group 3, i.e., Shoulder Protr-Elbow Flex
...
% for this group (Shoulder Protr-Elbow Flex), xopt == x0
xopt = x0;
...
% [Mean_MMT_Shoulder_Elbow] = Muscle_Moment_G3(x);
...
pro = pro_AddInput(pro, @()pdf_Sobol([R2-dev*abs(R2) R2+dev*abs(R2)]), 'X6');

pro = pro_AddInput(pro, @()pdf_Sobol([LF0-dev*abs(LF0) LF0+dev*abs(LF0)]), 'X7');
pro = pro_AddInput(pro, @()pdf_Sobol([Vmax_LF0-dev*abs(Vmax_LF0) Vmax_LF0+dev*abs(Vmax_LF0)]), 'X8');
pro = pro_AddInput(pro, @()pdf_Sobol([SO-dev*abs(SO) SO+dev*abs(SO)]), 'X9');
pro = pro_AddInput(pro, @()pdf_Sobol([PCSA-dev*abs(PCSA) PCSA+dev*abs(PCSA)]), 'X10');
pro = pro_AddInput(pro, @()pdf_Sobol([PA-dev*abs(PA) PA+dev*abs(PA)]), 'X11');



% set the model, and name it as 'model', to the project 
% the model is well-known as "Ishigami function" (see 3.0.1)
pro = pro_SetModel(pro, @(x)Muscle_Moment_G3(x), 'model');
...
% algorithm
Sfast_G3 = GSA_FAST_GetSi(pro);



save('Sfast_G3.mat','Sfast_G3');
