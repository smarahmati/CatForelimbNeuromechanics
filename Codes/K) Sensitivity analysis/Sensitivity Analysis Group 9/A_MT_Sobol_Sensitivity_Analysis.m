...
% Description:
% This script performs a sensitivity analysis of the mean maximum muscle-tendon (MT) moment to changes in 
% muscle parameters for the equivalent muscle in group 9, i.e., Shoulder Retractors-Elbow Extensors.
...
G = 9; % Muscle Group Number (G = 1 to 9)
...
% for this group (Shoulder Retr-Elbow Ext), xopt == x0
xopt = x0;
...
% x = [a1,a2,phi1,phi2,R1,R2, LF0,Vmax_LF0,SO,PCSA,PA];
% [Mean_MMT_Shoulder_Elbow] = Muscle_Moment_G9(x);
...
dev = 0.1;
pro = pro_AddInput(pro, @()pdf_Sobol([a1-dev*abs(a1) a1+dev*abs(a1)]), 'a1');
pro = pro_AddInput(pro, @()pdf_Sobol([a2-dev*abs(a2) a2+dev*abs(a2)]), 'a2');
pro = pro_AddInput(pro, @()pdf_Sobol([phi1-dev*abs(phi1) phi1+dev*abs(phi1)]), 'phi1');
pro = pro_AddInput(pro, @()pdf_Sobol([phi2-dev*abs(phi2) phi2+dev*abs(phi2)]), 'phi2');
pro = pro_AddInput(pro, @()pdf_Sobol([R1-dev*abs(R1) R1+dev*abs(R1)]), 'R1');
pro = pro_AddInput(pro, @()pdf_Sobol([R2-dev*abs(R2) R2+dev*abs(R2)]), 'R2');

pro = pro_AddInput(pro, @()pdf_Sobol([LF0-dev*abs(LF0) LF0+dev*abs(LF0)]), 'LF0');
pro = pro_AddInput(pro, @()pdf_Sobol([Vmax_LF0-dev*abs(Vmax_LF0) Vmax_LF0+dev*abs(Vmax_LF0)]), 'Vmax_LF0');
...
pro = pro_SetModel(pro, @(x)Muscle_Moment_G9(x), 'model');
...
% algorithm
Sfast_G9 = GSA_FAST_GetSi(pro);


save('Sfast_G9.mat','Sfast_G9');

%% plot
bar(Sfast_G9)
% a1,a2,phi1,phi2,R1,LF0,Vmax_LF0,SO,PCSA,PA
set(gca,'xticklabel',{'a1','a2','phi1','phi2','R1','R2','LF0','Vmax/LF0','SO','PCSA','PA'})
