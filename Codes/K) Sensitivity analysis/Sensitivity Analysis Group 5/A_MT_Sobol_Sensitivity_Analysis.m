...
% muscle parameters for the equivalent muscle in group 5, i.e., Elbow Flexor-Wrist Dorsiflexors
...
% [Mean_MMT_Elbow] = Muscle_Moment_G5(x);
...
pro = pro_AddInput(pro, @()pdf_Sobol([R1OW-dev*abs(R1OW) R1OW+dev*abs(R1OW)]), 'R1OW');
...
pro = pro_SetModel(pro, @(x)Muscle_Moment_G5(x), 'model');
...
% algorithm
Sfast_G5 = GSA_FAST_GetSi(pro);



save('Sfast_G5.mat','Sfast_G5');

%% plot
bar(Sfast_G5)
% a1,a2,phi1,phi2,R1,LF0,Vmax_LF0,SO,PCSA,PA
set(gca,'xticklabel',{'a1OW','a2OW','phi1OW','phi2OW','R1OW','a2WI','phi2WI','R1WI','LF0','Vmax/LF0','SO','PCSA','PA'})
