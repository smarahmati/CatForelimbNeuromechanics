...
cmn = 2;              % considered muscle number 
...
phi1 = phi1_ALL(MN,1);
phi2 = phi2_ALL(MN,1);
R1 = (R1_ALL(MN,1)*Sc_a1 + R1_ALL(MN,1)*Sc_a2)/2;
R2 = (R2_ALL(MN,1)*Sc_a1 + R2_ALL(MN,1)*Sc_a2)/2;
f1 = f1_ALL(MN,1);
f2 = f2_ALL(MN,1);
n1 = n1_ALL(MN,1);
n2 = n2_ALL(MN,1);
...
%% Write information
mn = 'Anconeus';                                       % muscle name
ml = MuscleLength_time_JA;                             % muscle length
mv = MuscleVelocity_time_JA;                           % muscle Velocity
ma = [time*0, MuscleMomentArm_time, time*0];           % muscle momemet arm
...
% hold on
 ...
 %------------------  left fore limbs -----------------------------------
 line([LSCP(ii,1) LSHL(ii,1)],[LSCP(ii,2) LSHL(ii,2)],'color',[0 0 0],'LineWidth',1,'LineStyle','-')
 hold on
 line([LSHL(ii,1) LELB(ii,1)],[LSHL(ii,2) LELB(ii,2)],'color',[0 0 0],'LineWidth',1,'LineStyle','-')
 hold on
 line([LELB(ii,1) LWRT(ii,1)],[LELB(ii,2) LWRT(ii,2)],'color',[0 0 0],'LineWidth',1,'LineStyle','-')
 hold on
 line([LWRT(ii,1) LMCP(ii,1)],[LWRT(ii,2) LMCP(ii,2)],'color',[0 0 0],'LineWidth',1,'LineStyle','-')
 hold on
 % line([LMCP(ii,1) LFT(ii,1)],[LMCP(ii,2) LFT(ii,2)],'color',[0 0 0],'LineWidth',1,'LineStyle','-')
 hold on
...
% Lmt1    
  Li = line([XO_time(ii,1) XP1_time(ii,1)],[YO_time(ii,1) YP1_time(ii,1)],'color',MC,'LineWidth',1,'LineStyle','-');
  hold on
  Li.Color=[MC,0.5];
...
% Lmt2   
  ...
  xunit = r * cos(th) + x;
  yunit = r * sin(th) + y;
  Cj2 = plot(xunit, yunit,'color',MC,'LineWidth',1,'LineStyle','-');
  Cj2.Color=[MC,0.5];
  hold on
  
  
% Lmt3   
  Li = line([XP2_time(ii,1) XI_time(ii,1)],[YP2_time(ii,1) YI_time(ii,1)],'color',MC,'LineWidth',1,'LineStyle','-');
  hold on  
  Li.Color=[MC,0.5];
  
else
% Lmt   
  Li = line([XO_time(ii,1) XI_time(ii,1)],[YO_time(ii,1) YI_time(ii,1)],'color',MC,'LineWidth',1,'LineStyle','-');
  hold on   
  Li.Color=[MC,0.5];
end
