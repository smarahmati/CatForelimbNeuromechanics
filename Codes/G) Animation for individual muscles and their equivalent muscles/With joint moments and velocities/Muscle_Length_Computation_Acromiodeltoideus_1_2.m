...
cmn = 1;                 % considered muscle number 
...
Lscp_Mo2D = MotionData.GenericCat.MLW.ScapulaLength;
...
SCP(:,1) = SHL(:,1) + Lscp_Mo2D*cos(q1-pi);
SCP(:,2) = SHL(:,2) + Lscp_Mo2D*sin(q1-pi);
...
Lscp_MS2D = MusculoskeletalData.MusculoskeletalData.Two_Dimension.Segment_Length.Scapula_Length;
...
Lscp_MS3D = MusculoskeletalData.MusculoskeletalData.Three_Dimension.Segment_Length.Scapula_Length;
...
Scale_Scap2D = Lscp_Mo2D/Lscp_MS2D;
...
%% Write information
mn = 'Acromiodeltoideus';                              % muscle name
