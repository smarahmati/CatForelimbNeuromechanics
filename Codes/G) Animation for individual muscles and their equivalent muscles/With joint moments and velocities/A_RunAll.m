...
% This M-file creates an animation of individual muscles as well as the equivalent muscles, including joint moments and velocities displayed as a circular array.
...
% arc arrow for joint moment
radius = 0.005;
center = [WRT(ii,1), WRT(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.Moments.MomWrist.Total(ii)/max(abs(MotionData.GenericCat.Moments.MomWrist.Total)))*360;
linewidth = 2;
colour = 'r';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);


radius = 0.005;
center = [ELB(ii,1), ELB(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.Moments.MomElbow.Total(ii)/max(abs(MotionData.GenericCat.Moments.MomElbow.Total)))*360;
linewidth = 2;
colour = 'r';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);
...
radius = 0.005;
center = [SHL(ii,1), SHL(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.Moments.MomShou.Total(ii)/max(abs(MotionData.GenericCat.Moments.MomShou.Total)))*360;
linewidth = 2;
colour = 'r';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);
...
% arc arrow for joint velocity
radius = 0.01;
center = [WRT(ii,1), WRT(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.AngularVelocities.VAWrist.Total(ii)/max(abs(MotionData.GenericCat.AngularVelocities.VAWrist.Total)))*360;
linewidth = 2;
colour = 'b';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);
...
center = [ELB(ii,1), ELB(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.AngularVelocities.VAElbow.Total(ii)/max(abs(MotionData.GenericCat.AngularVelocities.VAElbow.Total)))*360;
linewidth = 2;
colour = 'b';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);
...
center = [SHL(ii,1), SHL(ii,2)];
arrow_angle = 0; % Desired orientation angle in degrees
angle = (MotionData.GenericCat.AngularVelocities.VAShou.Total(ii)/max(abs(MotionData.GenericCat.AngularVelocities.VAShou.Total)))*360;
linewidth = 2;
colour = 'b';
arrowhead_angle = 30;
circular_arrow(figHandle, radius, center, arrow_angle, angle, colour, linewidth, arrowhead_angle);
